...
$(function () {
  ...
  $('script').each(function (index) {
    ...
    if ($this.text().indexOf('var mglistaccount') !== -1) {
      pos = $this.text().indexOf('var mglistaccount');
      end = $this.text().indexOf('];', pos);
      eval($this.text().substring(pos, end + 2));
      mglstacc = mglistaccount;
    }

    if ($this.text().indexOf('var mglistsync') !== -1) {
      pos = $this.text().indexOf('var mglistsync');
      end = $this.text().indexOf(';', pos);
  ...
  $.getScript('js/mirroricons.js', function (resp) {
    eval(resp);
    icons = mirroricons;
  });
  ...
  $('.forceuploadamr').click(function () {
    onButtonClickAsync($(this), replaceServerList);
  });

  $('.forcedownloadamr').click(function () {
    onButtonClickAsync($(this), replaceLocalList);
  });
});
