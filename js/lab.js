/*globals amrcsql, getMirrors, loadMenu, getMangaMirror*/
...
function displayNotification (wsData) {
  ...
  if (params.notificationtimer > 0) {
    setTimeout(notif.cancel, 1000 * params.notificationtimer);
  }
}
...
function modifyResult (idTest, res) {
  ...
  $('.res', elTest).empty();
  res.appendTo($('.res', elTest));
}

function modifyComment (idTest, comment) {
  var elTest = $('#' + idTest);

  $('.comment', elTest).empty();
  $('<span>' + comment + '</span>').appendTo($('.comment', elTest));
}
...
function testNPRetURL (urlLoad, libNP) {
...
function testChapters () {
  ...
  getMangaMirror($('#mirrorList').val()).getListChaps($('#lstMangas').val(), $('#lstMangas option:selected').text(), null, function (lst) {
    var res = $('<div></div>');
    var isOk = true;
